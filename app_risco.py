...
# título
st.title("Sistema Previsão de Concessão de Empréstimos - By Maicon Adone")
...
indice_inad = st.sidebar.number_input(
    "Índice de Inadimplência", value=data.indice_inad.mean())
anot_cadastrais = st.sidebar.number_input(
    "Anotações Cadastrais", value=data.anot_cadastrais.mean())
class_renda = st.sidebar.number_input(
    "Classificação da Renda", value=data.class_renda.mean())
saldo_contas = st.sidebar.number_input(
    "Saldo de Contas", value=data.saldo_contas.mean())
...
if btn_predict:
    result = model.predict(
        [[indice_inad, anot_cadastrais, class_renda, saldo_contas]])
